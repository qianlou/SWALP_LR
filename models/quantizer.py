...
def block_quantize(data, bits, mode, ebit=8, small_block="FC", block_dim="B"):
    ...
    #print(max_exponent)
    max_exponent=-max_exponent+(bits-2)
    max_exponent=min(max(max_exponent, -2**(ebit-3)), 2**(ebit-3)-1)
    #print(max_exponent)
    assert max_exponent >= -2**(ebit-1)
    assert max_exponent <= 2**(ebit-1)-1
    #i = data * 2**(-max_exponent+(bits-2))
    i = data * 2**(max_exponent)
    if mode == "stochastic":
        add_r_(i)
        i.floor_()
    elif mode == "nearest":
